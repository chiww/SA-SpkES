...
import sys
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, 'package')))
...
class BuildCommand(Command):
    ...
    def run(self):
        ...
        # 离线环境，使用本地packages代替pip install，直接复制所有的依赖包到目标地址；
        # 同时不再需要setup中的 packages 和 package_dir 配置
        try:
            shutil.copytree(os.path.join(self.build_dir, os.pardir, os.pardir, os.pardir, 'package'), os.path.join(self.build_dir, 'lib'))
        except Exception as e:
            print(e)
            print("package exists, do nothings")

        try:
            shutil.copytree(
                os.path.join(self.build_dir, os.pardir, os.pardir, os.pardir, 'SA-SpkES', 'package', 'default', 'data'),
                os.path.join(self.build_dir, 'default', 'data'))
        except Exception as e:
            print(e)
            print("package exists, do nothings")
...
try:
    setup(
        description='Search Elasticsearch in Splunk.',
        name=os.path.basename(project_dir),
        version='1.0.0',
        author='weiw.chi@gmail.com',
        author_email='weiw.chi@gmail.com',
        ...
        packages=[
            ...
            # 'lib.elasticsearch', 'lib.elasticsearch_dsl'
        ],
        package_dir={
            'dafault': os.path.join('default'),
            'default.data': os.path.join('default', 'data'),
            'default.data.ui': os.path.join('default', 'data', 'ui'),
            'default.data.ui.nav': os.path.join('default', 'data', 'ui', 'nav'),
            'default.data.ui.views': os.path.join('default', 'data', 'ui', 'views')
        },
        package_data={
            'bin': [
                os.path.join('package', 'bin', 'app.py'),
                os.path.join('package', 'bin', 'default.py'),
                os.path.join('package', 'bin', 'eslookup.py'),
                os.path.join('package', 'bin', 'essearch.py'),
                os.path.join('package', 'bin', 'escommand.py'),
            ],
            # os.path.join('bin', 'packages'): [
            #     os.path.join('package', 'bin', 'packages', '__init__.py'),
            #     os.path.join('package', 'bin', 'packages', 'validators.py'),
            #     os.path.join('package', 'bin', 'packages', 'spkmodifiers.py')
            # ]
        },
        data_files=[
